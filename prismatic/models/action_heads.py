...
class L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.model = MLPResNet(
            num_blocks=24, 
            input_dim=input_dim*ACTION_DIM,  # 把每维的特征向量拼起来
            # 那单步动作如果原本可以理解成 7 份特征：
            # 第 1 维动作的特征：长度 4096
            # 第 2 维动作的特征：长度 4096
            # 第 7 维动作的特征：长度 4096
            # 把它们 按最后一维拼起来，就变成一个长度：
            #[ 4096 \times 7 = 28672 ]
    ...
    def predict_action(
            self, 
            actions_hidden_states,  # (批次，层数，图像块数 + 动作块数，维度)
            proprio=None, 
        ...
        task_token_count = self.num_task_tokens
        if task_token_count is None:
            task_token_count = actions_hidden_states.shape[2] - NUM_TOKENS
        if task_token_count <= 0:
            raise ValueError(
                f"Expected positive task token count before action tokens, got {task_token_count}."
            )

        task_hidden_states = actions_hidden_states[:, :, :task_token_count, :]
        actions_hidden_states = actions_hidden_states[:, :, task_token_count:, :]

        # 创建动作特征容器，给“每一步动作 × 每个动作维度”准备一个槽位
        cond_actions_hidden_states = torch.zeros(
            # NUM_ACTIONS_CHUNK一次要预测的动作步数
...
class MLPResNet(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(input_dim, hidden_dim) # 把输入特征 升维映射到隐藏层维度
        self.relu = nn.ReLU()
        ...
        for _ in range(num_blocks):
            if use_pro_version:
                ...
                self.mlp_resnet_blocks.append(MLPResNetBlock(dim=hidden_dim)) # 堆叠深度残差块
                
        self.layer_norm2 = nn.LayerNorm(hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, output_dim) # 最后映射成 动作维度


    def forward(self, x, h_a=None, h_t=None, p= None):
        # ht 图像特征 
        # ha 动作特征
        # x: (batch_size, input_dim) 初始动作
        x = self.layer_norm1(x)  # shape: (batch_size, input_dim)
        x = self.fc1(x)  # shape: (batch_size, hidden_dim)
        x = self.relu(x)  # shape: (batch_size, hidden_dim)
        for i, block in enumerate(self.mlp_resnet_blocks):
            # i表示这是第几层的特征，输入到Bridge Attention里
...
class MLPResNetBlock(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.o_proj = nn.Linear(dim, dim) # o_proj = 把 8 个头的信息揉成一个最终流畅特征
    ...
    def forward(self, x, h_t=None, h_a=None, p=None):
        ...
        if p is not None:
            conditions.append(p)
        # 把机器人动作特征和 proprioception 合并起来，作为条件特征
        ...
        output = self.o_proj(output) #残差连接
