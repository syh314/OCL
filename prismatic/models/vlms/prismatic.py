    ...
    def __init__(
        ...
        **kwargs,
        ...
        for trigger_string in ["True", "False", "Yes", "No"] + [chr(ord("A") + i) for i in range(26)]:
            token_idx_list = self.llm_backbone.tokenizer.encode(trigger_string, add_special_tokens=False) # 返回的是 list（因为有可能一个词拆成多个 token）
            assert len(token_idx_list) == 1, f'String "{trigger_string}" is tokenized as more than one token!' #  必须保证这个字符串只对应一个 token
