...
class GenerateConfig:
    ...
    save_rollout_videos: bool = True                 # Whether to save rollout MP4s for every episode
    summary_json_path: Optional[str] = None          # Optional path for structured eval summary JSON
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    summary_json_path = Path(cfg.summary_json_path) if cfg.summary_json_path is not None else Path(local_log_filepath).with_suffix(".json")
    summary_payload = {
    ...
    log_message(f"Saved eval summary JSON to {summary_json_path}", log_file)
